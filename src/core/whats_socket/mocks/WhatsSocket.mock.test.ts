...
describe("WhatsSocketMock Generally", () => {
  ...
  it("WhenSendingMsgsThroughSocket_MustBeSent", async () => {
    ...
    await socketMock._SendSafe(`chatIdtest${WhatsappGroupIdentifier}`, { text: "First messsage" });
    expect(socketMock.SentMessagesThroughQueue.length).toBe(1);
    await socketMock._SendSafe(`chatIdtest${WhatsappGroupIdentifier}`, { text: "Second messsage" });
    expect(socketMock.SentMessagesThroughQueue.length).toBe(2);
    await socketMock._SendSafe(`chatIdtest${WhatsappGroupIdentifier}`, { text: "Third messsage" });
  ...
  it("Clear_WhenSocketMockHasBeenUseALot_ShouldCleanItself", async () => {
    ...
    socketMock.onIncomingMsg.Subscribe(() => {});
    ...
    expect(socketMock.onIncomingMsg.Length).toBe(1);
    ...
    expect(socketMock.onIncomingMsg.Length).toBe(0);
