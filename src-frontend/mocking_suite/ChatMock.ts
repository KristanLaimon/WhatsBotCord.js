import { type WhatsBotOptions, BotUtils_GenerateOptions } from "../bot/bot.js";
import type { IChatContextConfig } from "../bot/internals/ChatContext.js";
import Myself_Submodule_Status from "../bot/internals/ChatContext.myself.status.js";
import CommandsSearcher, { CommandType } from "../bot/internals/CommandsSearcher.js";
...
import type { WhatsSocketMockMsgSent } from "../whats_socket/mocks/types.js";
...
export type MockEnqueueParamsMultimediaMinimal = MockEnqueueParamsMinimal & { imgContentBufferMock?: Uint8Array };
...
export default class ChatMock {
  ...
  public EnqueueIncoming_Document(urlDocument: string, fileName: string, opts?: MockEnqueueParamsDocument): void {
    const documentMsg: WhatsappMessage = MsgFactory_Document(this.ChatId, this.ParticipantId_LID, urlDocument, {
      fileName: fileName,
      mimetype: opts?.mimeType ?? MimeTypeHelper_GetMimeTypeOf({ source: fileName }) ?? "application/pdf",
      pushName: opts?.pushName,
    });
    if (opts?.imgContentBufferMock) {
      this._chatContextMock.EnqueueMediaBufferToReturn(opts.imgContentBufferMock);
  ...
  public async StartChatSimulation(): Promise<void> {
    ...
    try {
      await this._command.run(
        ...
        {
          InternalSocket: this._socketMock as any,
          Myself: {
            Status: new Myself_Submodule_Status(this._socketMock as any),
            Bot: {
   ...
   */
  @autobind
  public SetWaitMsgBufferToReturnMock(anyBuffer: Uint8Array) {
